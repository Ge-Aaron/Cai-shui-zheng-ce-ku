...
3. 黑色窗口会显示内置的仓库地址（`https://github.com/Ge-Aaron/Cai-shui-zheng-ce-ku.git`），**无需粘贴**，脚本自动初始化并拉取/推送
4. 接下来二选一：
...
### Render 免费层会收费吗？
- **750 小时免费实例额度用完**：不会自动扣费，Render 会**暂停你的免费服务到下个月 1 号重置**，不会转付费。
- **15 分钟无访问自动休眠**：这是帮你省免费小时数的设计，休眠期间不消耗额度；下次访问约 1 分钟唤醒。
- **真正可能产生费用的两种情况**（都很少见）：
  1. **出站带宽**超出月包含额度（免费层含 100GB/月，小站基本用不完）。
  2. **构建流水线分钟**超出额度（免费层含 500 分钟/月，正常只部署/更新几次，用不完）。
  这两项**只有在你绑定了信用卡时才会补充计费**；没绑卡则直接暂停服务。
- **绑信用卡时的 $1 预授权**：是银行安全验证，会退款，不是 Render 扣款。
- **想彻底避免任何意外扣费**：进 Render → Billing → 设置 **Monthly spend limit** 为你能接受的上限（如 $0 或 $1），即可锁住。
- 你的财税政策查询站流量很小，正常使用时**几乎不可能产生费用**。
